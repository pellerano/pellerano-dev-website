...
import { NAVBAR_THEME_MAPPING, TAILWIND_BREAKPOINTS } from '../contants';
...
function NavButton({ label, isActive, onClick, orientation = 'vertical', variant }) {
  const isVertical = orientation === 'vertical';

  return (
    <button
      onClick={onClick}
      className={`${variant === 'light' ? 'text-white' : 'text-black'} cursor-pointer flex ${
        isVertical ? 'flex-col items-center p-6' : 'items-center gap-4'
      }`}
    >
      {!isVertical && (
        <span
          className={`w-1 h-1 rounded-full ${variant === 'light' ? 'bg-white' : 'bg-black'} transition-all duration-200 ${
      ...
      {isVertical && (
        <span
          className={`w-1 h-1 rounded-full transition-all duration-200 ${variant === 'light' ? 'bg-white' : 'bg-black'} ${isActive ? 'opacity-100 scale-100' : 'opacity-0 scale-0'}`}
/>
      )}
    </button>
  );
...
function DesktopNavbar({ activeLink, variant = 'light' }) {
...
function MobileNavbar({ activeLink, variant = 'light' }) {
  ...
  return (
    <Collapsible open={open} onOpenChange={setOpen}>
      <CollapsibleTrigger asChild className="p-6 cursor-pointer">
        <div className={`${variant === 'light' ? 'text-white' : 'text-black'}`}>
          <Pellerano />
        </div>
      </CollapsibleTrigger>
...
export default function Navbar({ active = 'home' }) {
  ...
  const pathname = usePathname();
  const variant = NAVBAR_THEME_MAPPING[pathname]

  return (
    <div className="fixed left-1/2 -translate-x-1/2 top-4 z-50">
      <GlassCard>
        {isMobile ? (
          <MobileNavbar {...{activeLink: active, variant}} />
        ) : (
          <DesktopNavbar {...{activeLink: active, variant}} />
        )}
      </GlassCard>
    </div>
  );
